...
type User struct {
    ...
    Password_hash string        `orm:"size(128)" json:"password"`     // 用户密码(加密后)
    Mobile        string        `orm:"size(11)";unique json:"mobile"` // 手机号码
    Real_name     string        `orm:"size(32)" json:"real_name"`     // 真实姓名（实名认证）
    Id            string        `orm:"size(20)" json:"id"`            // 身份证号码
    Avatar_url    string        `orm:size(256) json:"avatar_url"`     // 用户头像路径
    Houses        []*House      `orm:reverse(many) json:"houses"`     // 用户发布的房屋信息
    Orders        []*Orderhouse `orm: reverse(mayn) json:"orders"`    // 用户下的订单
}
...
type House struct {
    ...
    Room_count      int           `orm:"default(1)" orm:"default("")" json:"room_count"`    // 房间数量
    Acreage         int           `orm:"default(0)" json:"acreage"`                         // 房屋总面积
    Unit            string        `orm:"size(32)" orm:"default("")" json:"unit"`            // 房屋单元，如 两室一厅
    Capacity        int           `orm:"default(1)" json:"capacity"`                        // 房屋容纳的人数
    ...
    Order_count     int           `orm:"default(0)" json:"order_count"`                     // 预定完成的该房间订单数
    Index_image_url string        `orm:"size(256)" orm:"default(0)" json:"index_image_url"` // 房屋主图片路径
    Facilities      []*Facility   `orm:"reverse(many)" json:"facilities"`                   // 房屋设施
    Images          []*HouseImage `orm:"reverse(many)" json:"jmg_urls"`                     // 房屋的图片
    Orders          []*OrderHouse `orm:"reverse(many)" json:"orders"`                       // 房屋的订单
    Citme           time.Time     `orm:"auto_now_add;type(datetime)" json:"citme"`
}
...
// 房屋列表页面每页显示条目数
var HOUSE_LIST_PAGE_CAPTCITY int = 2
...
func (this *House) To_house_info() interface{} {
    house_info := map[string]interface{}{
        ...
        "ctime":       this.Citme.Format("2006-01-02 15:04:05"),
    }
    return house_info
}
...
func (this *House) To_one_house_desc() interface{} {
    ...
    orders := []OrderHouse{}
    o := orm.NewOrm()
    order_num, err := o.QueryTable("order_house").Filter("house_id", this.Id).Filter("status", ORDER_STATUS_COMPLETE)
    if err != nil {
...
/*区域信息 table_name = area */ // 区域信息是需要手工添加到数据库中
type Area struct {
    Id     int      `json:"aid"`                    // 区域编号  1   2
    Name   string   `orm:"size(32)" json:"aname"`   // 区域名字 昌平 海淀
    Houses []*House `orm:"rel(many)" json:"houses"` // 区域所有的房屋 与房屋表关联
}

// 设施信息 table_name = "facility" 设施信息需要提前手工添加
type Facility struct {
    Id     int      `json:"fid"`     // 设施编号
    Name   string   `orm:"size(32)`  // 设施名称
    Houses []*House `orm:"rel(m2m)"` // 都有那些房屋有此设施？ 与房屋表关联
}

/*房屋图片 table_name = "house_image"*/
type HouseImage struct {
    Id    int    `json:"house_image_id"`         // 图片id
    Url   string `orm:"size(256)" json:"url"`    // 图片url 存放房屋的图片
...
type OrderHouse struct {
    ...
    House       *House    `orm:"rel(fk)" json:"house_id"`              // 预定的房间编号 与房屋信息表进行关联
    Begin_date  time.Time `orm:"type(datetime)"`                       // 预定的开始时间
...
func init() {
    ...
    // set default dabase
    // 连接数据库(默认参数，mysql数据库，"数据库用户名:数据库密码@tcp("+数据库地址+"："+数据库端口+")/库名？/格式，默认值")
    orm.RegisterDataBase("default", "mysql", "fuckit:123456@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/go1micro?charset=utf8",
        30)
    ...
    // create table
    // 第一个是别名
    // 第二个 是否强制替换模块 如果表变更就将false改为true，之后再换回来表就更新好
    // 第三个参数是如果没有则同步或创建
